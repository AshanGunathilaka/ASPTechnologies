...
import { fetchInvoice } from "../../../api/suppliers";
...
export default function InvoicePaymentsPage() {
  const { supplierId, invoiceId } = useParams();
  const { auth } = useAuth();
  ...
  const loadData = useCallback(async () => {
    try {
      const [invoiceRes, paymentsRes] = await Promise.all([
        fetchInvoice(token, supplierId, invoiceId),
        fetchPayments(token, supplierId, invoiceId),
      ]);
      setInvoice(invoiceRes.data);
      setPayments(paymentsRes.data);

      // Prefill amount with remaining balance based on original grand total (items - discount), not reduced by payments
      const subtotal = (invoiceRes.data?.items || []).reduce(
        (s, it) => s + (Number(it.quantity) * Number(it.unitPrice) || 0),
        0
      );
      const discount = Number(invoiceRes.data?.discount || 0);
      const originalGrand = subtotal - discount;
      const paid = (paymentsRes.data || []).reduce(
        (a, p) => a + Number(p.amount || 0),
        0
      );
      const remaining = Math.max(0, Number((originalGrand - paid).toFixed(2)));
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await createPayment(token, supplierId, invoiceId, formData);
  ...
  const handleDelete = async (paymentId) => {
    ...
    try {
      await deletePayment(token, supplierId, invoiceId, paymentId);
  ...
  if (!invoice)
    return (
      <p className="p-6 text-gray-600 animate-pulse">
        Loading invoice and payments...
      </p>
    );

  // Always compute the original grand total from items and discount, independent of payments
  const subtotal = (invoice.items || []).reduce(
    (s, it) => s + (Number(it.quantity) * Number(it.unitPrice) || 0),
    0
  );
  const originalGrand = subtotal - Number(invoice.discount || 0);
    ...
    >
      ...
      <div className="max-w-5xl mx-auto bg-white p-6 rounded-2xl shadow-lg border border-yellow-200">
        <h1 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 via-amber-600 to-yellow-700 drop-shadow-md mb-2">
          Payments for Invoice #{invoice.number}
        </h1>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={() =>
            navigate(`/admin/suppliers/${supplierId}/invoices/${invoiceId}`)
          }
